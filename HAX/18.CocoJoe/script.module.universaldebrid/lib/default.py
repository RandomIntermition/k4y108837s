...
tmdb_test = str(xbmcaddon.Addon('script.module.universaldebrid').getSetting("tmdb_test"))

ADDON_PATH = xbmc.translatePath('special://home/addons/script.module.universaldebrid/')
ICON = ADDON_PATH + 'icon.png'
FANART = ADDON_PATH + 'fanart.jpg'

USERDATA_PATH = xbmc.translatePath('special://home/userdata/addon_data')
ADDON_DATA = os.path.join(USERDATA_PATH,'script.module.universaldebrid')
...
scrapers_path = xbmc.translatePath('special://home/addons/script.module.universaldebrid/lib/universaldebrid/scraperplugins')
...
if mode == "DisableAll":
    scrapers = sorted(
        universaldebrid.relevant_scrapers(include_disabled=True), key=lambda x: x.name.lower())
    for scraper in scrapers:
        key = "%s_enabled" % scraper.name
        xbmcaddon.Addon('script.module.universaldebrid').setSetting(key, "false")
    sys.exit()
elif mode == "EnableAll":
    scrapers = sorted(
        universaldebrid.relevant_scrapers(include_disabled=True), key=lambda x: x.name.lower())
    for scraper in scrapers:
        key = "%s_enabled" % scraper.name
        xbmcaddon.Addon('script.module.universaldebrid').setSetting(key, "true")
    sys.exit()
elif mode == "Deletelog":
    from universaldebrid.common import Del_LOG
    Del_LOG()
    sys.exit()
...
def tmdb_movies(html):
    ...
    for url, name in match:
        ...
        for title,year in match2:
            title = title[:-1]
            movie_links_scraper = universaldebrid.scrape_movie(title, year, '')
...
def single_test(count, index):
    if count==5:
        ...
        if dialog.yesno("Universalscrapers Testing Mode", 'Run next Movie?',Movies['title']+' ('+Movies['year']+')'):
            movie_links_scraper = universaldebrid.scrape_movie(Movies['title'], Movies['year'], Movies['imdb'])
            movie_links_scraper = movie_links_scraper()
        ...
        tv_shows['title']+' ('+tv_shows['year']+') S'+tv_shows['season']+'E'+tv_shows['episode']):
            episode_links_scraper = universaldebrid.scrape_episode(tv_shows['title'], tv_shows['show_year'], tv_shows['year'], tv_shows['season'], tv_shows['episode'], tv_shows['imdb'],'')
...
def full_test():
    ...
    for item in movies:
        ...
        if pDialog.iscanceled():
            break
        movie_links_scraper = universaldebrid.scrape_movie(item['title'], item['year'], item['imdb'])
        movie_links_scraper = movie_links_scraper()
    ...
    for item in shows:
        ...
        if pDialog.iscanceled():
            break
        episode_links_scraper = universaldebrid.scrape_episode(item['title'], item['show_year'], item['year'], item['season'], item['episode'], item['imdb'],'')
        episode_links_scraper = episode_links_scraper()

...
# Doko'S Dupe Checker for cleaning results.
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=30):
        ##### DIALOG ###################################################################
        ## enabled module name
        ## LIMITLESS INFINITE  # bypass.limit = ignore lambda settings

        scrapertag = "FAULT"

        if not control.setting('module.version') == '':
            scrapertag = "    Using: [" + self.module_name + "]"

        if control.setting('bypass.limit') == 'true':
                scrapertag = scrapertag + " [All]"
        else:
            scrapertag = scrapertag + " [Custom]"

        if control.setting('sources.dont.filter') == 'true':
                scrapertag = scrapertag + "[Limitless]"

        progressDialog = control.progressDialog if control.setting('progress.dialog') == '0' else control.progressDialogBG
        progressDialog.create(control.addonInfo('name') + "..." + scrapertag, '')
        progressDialog.update(0)
        ...
        if control.setting('provider.version') == "LambdaScrapers":
        ...
        elif control.setting('provider.version') == "None":
            try: sourceDict = [(i[0], i[1], control.addon("script.module.lambdascrapers").getSetting('provider.' + i[0])) for i in sourceDict]
            except: sourceDict = [(i[0], i[1], 'true') for i in sourceDict]
        else:
        ...
        #ORIG
        pdiag_format = ' TOP: %s | 4K: %s | 1080p: %s | 720p: %s | SD: %s | %s: %s'.split('|')
        pdiag_bg_format = 'TOP:%s(%s)|4K:%s(%s)|1080p:%s(%s)|720p:%s(%s)|SD:%s(%s)|T:%s(%s)'.split('|')
        # 4K
        pdiag_format_4k = ' TOP: %s | 4K: %s | 1080p: %s | 720p: %s | SD: %s | %s: %s'.split('|')
        pdiag_bg_format_4K = 'TOP:%s(%s)|4K:%s(%s)|1080p:%s(%s)|720p:%s(%s)|SD:%s(%s)|T:%s(%s)'.split('|')
        # 1440p
        pdiag_format_1440 = ' TOP: %s | 1080p: %s | 720p: %s | SD: %s | %s: %s'.split('|')
        pdiag_bg_format_1440 = 'TOP:%s(%s)|1080p:%s(%s)|720p:%s(%s)|SD:%s(%s)|T:%s(%s)'.split('|')
        # 1080p
        pdiag_format_1080 = ' TOP: %s | 1080p: %s | 720p: %s | SD: %s | %s: %s'.split('|')
        pdiag_bg_format_1080 = 'TOP:%s(%s)|1080p:%s(%s)|720p:%s(%s)|SD:%s(%s)|T:%s(%s)'.split('|')
        # 720p
        pdiag_format_720 = ' TOP: %s | 720p: %s | SD: %s | %s: %s'.split('|')
        pdiag_bg_format_720 = 'TOP:%s(%s)|720p:%s(%s)|SD:%s(%s)|T:%s(%s)'.split('|')
        # SD / 480p
        pdiag_format_sd = ' SD: %s | %s: %s'.split('|')
        pdiag_bg_format_sd = 'SD:%s(%s)|T:%s(%s)'.split('|')
        ...
        for i in range(0, 4 * timeout):
            try:
                ...
                if len(self.sources) > 0:
                    if quality in ['0']:
                        source_top = len([e for e in self.sources if e['quality'] == 'TOP' and e['debridonly'] == False])
                    ...
                    total = source_top + source_4k + source_1080 + source_720 + source_sd

                    if debrid_status:
                        if quality in ['0']:
                            for d in debrid_list:
                                d_source_top = len([e for e in self.sources if e['quality'] == 'TOP' and d.valid_url('', e['source'])])
                        ...
                        d_total = source_top + d_source_4k + d_source_1080 + d_source_720 + d_source_sd

                if debrid_status:
                    d_top_label = total_format % ('red', d_source_top) if d_source_top == 0 else total_format % ('lime', d_source_top)
                ...
                source_top_label = total_format % ('red', source_top) if source_top == 0 else total_format % ('lime', source_top)
                        ...
                        if debrid_status:
                            if quality in ['0']:
                                if not progressDialog == control.progressDialogBG:
                                    line1 = ('%s:' + '|'.join(pdiag_format_4k)) % (string6, d_top_label, d_4k_label, d_1080_label, d_720_label, d_sd_label, str(string4), d_total_label)
                                    line2 = ('%s:' + '|'.join(pdiag_format_4k)) % (string7, source_top_label, source_4k_label, source_1080_label, source_720_label, source_sd_label, str(string4), source_total_label)
                                    print line1, line2
                                else:
                                    line1 = '|'.join(pdiag_bg_format_4k) % (source_top_label, d_top_label, source_4k_label, d_4k_label, source_1080_label, d_1080_label, source_720_label, d_720_label, source_sd_label, d_sd_label, source_total_label, d_total_label)
                            elif quality in ['1']:
                                if not progressDialog == control.progressDialogBG:
                                    line1 = ('%s:' + '|'.join(pdiag_format_1440)) % (string6, d_top_label, d_1080_label, d_720_label, d_sd_label, str(string4), d_total_label)
                                    line2 = ('%s:' + '|'.join(pdiag_format_1440)) % (string7, source_top_label, source_1080_label, source_720_label, source_sd_label, str(string4), source_total_label)
                                else:
                                    line1 = '|'.join(pdiag_bg_format_1440) % (source_top_label, d_top_label, source_1080_label, d_1080_label, source_720_label, d_720_label, source_sd_label, d_sd_label, source_total_label, d_total_label)
                            elif quality in ['2']:
                                if not progressDialog == control.progressDialogBG:
                                    line1 = ('%s:' + '|'.join(pdiag_format_1080)) % (string6, d_top_label, d_1080_label, d_720_label, d_sd_label, str(string4), d_total_label)
                                    line2 = ('%s:' + '|'.join(pdiag_format_1080)) % (string7, source_top_label, source_1080_label, source_720_label, source_sd_label, str(string4), source_total_label)
                                else:
                                    line1 = '|'.join(pdiag_bg_format_1080) % (source_top_label, d_top_label, source_1080_label, d_1080_label, source_720_label, d_720_label, source_sd_label, d_sd_label, source_total_label, d_total_label)
                            elif quality in ['3']:
                                if not progressDialog == control.progressDialogBG:
                                    line1 = ('%s:' + '|'.join(pdiag_format_720)) % (string6, d_top_label, d_720_label, d_sd_label, str(string4), d_total_label)
                                    line2 = ('%s:' + '|'.join(pdiag_format_720)) % (string7, source_top_label, source_720_label, source_sd_label, str(string4), source_total_label)
                                else:
                                    line1 = '|'.join(pdiag_bg_format_720) % (source_top_label, d_top_label, source_720_label, d_720_label, source_sd_label, d_sd_label, source_total_label, d_total_label)
                            else:
                                if not progressDialog == control.progressDialogBG:
                                    line1 = ('%s:' + '|'.join(pdiag_format_sd)) % (string6, d_sd_label, str(string4), d_total_label)
                                    line2 = ('%s:' + '|'.join(pdiag_format_sd)) % (string7, source_sd_label, str(string4), source_total_label)
                                else:
                                    line1 = '|'.join(pdiag_bg_format_sd) % (source_sd_label, d_sd_label, source_total_label, d_total_label)
                        else:
                            if quality in ['0']:
                                line1 = '|'.join(pdiag_format_4k) % (source_top_label, source_4k_label, source_1080_label, source_720_label, source_sd_label, str(string4), source_total_label)
                            elif quality in ['1']:
                                line1 = '|'.join(pdiag_format_1440) % (source_top_label, source_1080_label, source_720_label, source_sd_label, str(string4), source_total_label)
                            elif quality in ['2']:
                                line1 = '|'.join(pdiag_format_1080) % (source_top_label, source_1080_label, source_720_label, source_sd_label, str(string4), source_total_label)
                            elif quality in ['3']:
                                line1 = '|'.join(pdiag_format_720) % (source_top_label, source_720_label, source_sd_label, str(string4), source_total_label)
                            else:
                                line1 = '|'.join(pdiag_format_sd) % (source_sd_label, str(string4), source_total_label)
...
###   dupe checker
        try:
            if control.setting('sources.remove.dupes') == 'true':
        ...
        if quality in ['0', '1', '2', '3']: filter += [i for i in self.sources if i['quality'] == 'TOP' and 'debrid' in i]
        if quality in ['0', '1', '2', '3']: filter += [i for i in self.sources if i['quality'] == 'TOP' and not 'debrid' in i and 'memberonly' in i]
        if quality in ['0', '1', '2', '3']: filter += [i for i in self.sources if i['quality'] == 'TOP' and not 'debrid' in i and not 'memberonly' in i]
        ...
        for i in range(len(self.sources)):
            ...
            if t:
                if q in ['TOP', '4K', '1440p', '1080p', '720p']: label += '%s | [B][I]%s [/I][/B] | [I]%s[/I] | %s' % (s, q, t, f)
                elif q == 'SD': label += '%s | %s | [I]%s[/I]' % (s, f, t)
                else: label += '%s | %s | [I]%s [/I] | [I]%s[/I]' % (s, f, q, t)
            else:
                if q in ['TOP', '4K', '1440p', '1080p', '720p']: label += '%s | [B][I]%s [/I][/B] | %s' % (s, q, f)
    ...
    def sourcesDirect(self, items):
        ...
        if control.setting('autoplay.sd') == 'true':
            items = [i for i in items if not i['quality'] in ['TOP', '4K', '1440p', '1080p', 'HD']]
    ...
    def getConstants(self):
        ...
        ## BELOW - CHECKs AND UPDATEs SETTINGS BEFORE EVERY SEARCH.
        
        ## addon ID's (skips if not installed)
        placenta_addonid = xbmcaddon.Addon(id='plugin.video.placenta')
        
        try:
            lambda_addonid = xbmcaddon.Addon(id='script.module.lambdascrapers')
        except:pass
                

        # ORIGINAL SETTINGS (from placenta)
        placenta_providerset = placenta_addonid.getSetting('provider.version') ## sets 'provider'         aka lambdascrapers
        ...
        # LAMBDA enabled
        if control.setting('provider.version') == "LambdaScrapers":
            ## BIND SETTINGS (module version and bypass limit)
            lambda_addonid.setSetting('module.provider', placenta_scraperset)
            try:
                lambda_addonid.setSetting('module.lasthope', placenta_extrainclude)
                lambda_addonid.setSetting('bypass.limit', placenta_bypasslimit)
                lambda_addonid.setSetting('sources.dont.filter', placenta_dontfilter)
            except:pass
            ...
            try:
                self.module_name = placenta_scraperset
            except:
                self.module_name = 'Unknown'


        # NONE enabled (use as example template)
        if control.setting('provider.version') == "None":
            ## BIND SETTINGS (module version and bypass limit)
            lambda_addonid.setSetting('module.provider', placenta_scraperset)
            try:
                lambda_addonid.setSetting('module.lasthope', placenta_extrainclude)
        ...
        #### ABOVE - chooses scrapers and binds settings

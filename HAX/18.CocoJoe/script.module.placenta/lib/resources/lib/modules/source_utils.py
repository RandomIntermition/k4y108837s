...
### BELOW from scrubs, needed to stop hosts with limit failing
### edited to remove need for adding setting to limit scrapers to one result per host
### CURRENTLY defaults to false ie disabled ie do not limit results by host
def limit_hosts():
    enabled = "false"
    if enabled == 'true':
        return True

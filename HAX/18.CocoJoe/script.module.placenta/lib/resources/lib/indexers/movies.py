...
class movies:
    def __init__(self):
        ...
        #                    'https://www.imdb.com/search/title?title_type=feature&num_votes=1000,&release_date=date[365],date[0]&sort=release_date_us,desc&count=40&start=1'
       ...
       ######################### TRAKT AND IMDB URLS # TRAKT LINKS # personal lists
        ...
        self.traktcomingsoon_link = 'https://api.trakt.tv/movies/anticipated?limit=80&page=1&sort=added,asc'
        self.traktlatestrelease_link = 'https://api.trakt.tv/users/giladg/lists/latest-releases/items?limit=80&page=1&sort=added,asc'
        self.trakttoptenweekly_link = 'https://api.trakt.tv/users/giladg/lists/top-10-movies-of-the-week/items?limit=40&page=1&sort=added,asc'
    ...
    def certifications(self):
        ...
        #for i in certificates: self.list.append({'name': str(i), 'url': self.certification_link % str(i).replace('-', '-').lower(), 'image': 'certificates.png', 'action': 'movies'})
        self.addDirectory(self.list)
        return self.list
...
######### ABOVE AND BELOW HAVE BEEN COMMENTED OUT, REMOVED FROM CONTEXT MENU
...
######                if traktCredentials == True:               ##### REMOVED AS NEVER WORKED
######                    cm.append(("[COLOR blue][B]TRAKT[/B][/COLOR] Add to Collection", 'RunPlugin(%s?action=traktManager&name=addtocollection_%s&imdb=%s&content=movie)' % (sysaddon, sysname, imdb)))

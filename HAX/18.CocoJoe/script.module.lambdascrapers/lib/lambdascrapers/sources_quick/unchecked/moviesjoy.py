# -*- coding: UTF-8 -*-
# -Cleaned and Checked on 03-19-2019 by JewBMX in Scrubs.
...
class source:
    def __init__(self):
        ...
        self.BASE_URL = 'https://www.moviesjoy.to'
    ...
    def sources(self, data, hostDict, hostprDict):
        try:
            ...
            id, movie_id = re.search(b"""id.*?['"](.*?)['"].*?movie_id.*?['"](.*?)['"]""", htmlContent[dataIndex : dataIndex+200], re.DOTALL) . groups()
            session.cookies['view-' + str(id)] = 'true' # A cookie that they use.
            session.headers['Referer'] = data['url'].replace('.html', '/watching.html')
            r = self._sessionRequest(self.BASE_URL + (self.SERVER_PATH % (id, movie_id)), session, 0.7, data = None, ajax = True)
            if not r.ok:
            ...
            def _sourcesGen():
                ...
                for div in divs:
                    if isEpisode:
                        ...
                        li = div.li
                    resolveData = { 'data-id': li['data-id'], 'data-server': li['data-server'], 'UA': userAgent, 'referer': referer, 'cookies': cookies }
                    if 'direct' in li['onclick']:
                        # Resolve a direct source right now. There's usually only one anyway, Alphabet (Google Video).
                        r = self._sessionRequest(self.BASE_URL + (self.DIRECT_PATH + li['data-id'] + '-' + li['data-server']), session, delayAmount = 1.0, data = None, ajax = True)
                        hostName = div.span.text
                        # Multiple qualities in the direct source.
                        for quality in r.json()['playlist'][0]['sources']:
                            yield {'source': hostName, 'quality': quality['label'], 'language': 'en', 'url': quality['file'] + directHeaders, 'direct': True, 'debridonly': False}
                    else:
                        yield {'source': div.span.text, 'quality': 'SD', 'language': 'en', 'url': { 'data-id': li['data-id'], 'data-server': li['data-server'], 'UA': userAgent, 'referer': referer, 'cookies': cookies }, 'direct': False, 'debridonly': False}
    ...
    def resolve(self, data):
        ...
        session = self._createSession(data)
        r = self._sessionRequest(self.BASE_URL + (self.EMBED_PATH + data['data-id'] + '-' + data['data-server']), session, delayAmount = 1.2, data = None, ajax = True)
        if not r.ok:
    ...
    def _getSearchData(self, title, aliases, year, session, season, episode):
        try:
            ...
            r = self._sessionRequest(self.BASE_URL + self.QUICK_SEARCH_PATH, session, 1.2, {'keyword': query}, ajax=True)
            if not r.ok:
                return None
            possibleTitles = set((title.lower(),) + tuple((alias['title'].lower() for alias in aliases) if aliases else ()))
            ...
            if bestURL:
                return { 'url': bestURL, 'episode': episode, 'UA': session.headers['User-Agent'], 'referer': self.BASE_URL + '/', 'cookies': session.cookies.get_dict() }
            else:
    ...
    def _debug(self, name, val=None):
        import xbmc
        xbmc.log('MOVIESJOY-NET Debug > %s %s' % (name, (val if isinstance(val, str) else (repr(val) if val else ''))), xbmc.LOGWARNING)
